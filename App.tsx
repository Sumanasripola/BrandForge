  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-blue-100 text-slate-900 pb-20">
      ...
      <main className="container mx-auto px-4 -mt-12">
        {/* Form Section */}
        <section className="max-w-3xl mx-auto bg-white rounded-3xl shadow-xl border border-slate-200 p-8 md:p-10 mb-16 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex justify-between items-center mb-8">
              ...
              <button 
                ...
                className="flex items-center gap-2 text-sm font-bold text-indigo-600 hover:text-indigo-700 bg-indigo-50 px-4 py-2 rounded-full transition-all border border-indigo-100 hover:shadow-sm"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>
                {inputs.personalitySummary ? 'Soul Defined' : 'Brand Personality Quiz'}
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input 
                    type="text" 
                    name="industry"
                    placeholder="e.g. HealthTech, Sustainable Fashion"
                ...
                <div>
                  ...
                  <select 
                    ...
                    value={inputs.tone}
                    onChange={handleInputChange}
                    className="w-full px-4 py-3 rounded-lg border border-slate-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all bg-white"
                  >
              ...
              <div>
                ...
                <input 
                  type="text" 
                  name="targetAudience"
                  placeholder="e.g. Professional Developers, Busy Parents"
              ...
              <div>
                ...
                <textarea 
                  ...
                  rows={4}
                  placeholder="What are you building? Tell us your unique mission..."
              ...
              {inputs.personalitySummary && (
                <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-100 animate-in slide-in-from-left-4 duration-300">
              ...
              <Button 
                type="submit" 
                className="w-full py-4 text-lg bg-indigo-600 hover:bg-indigo-700 shadow-indigo-200 shadow-lg" 
                isLoading={loading}
              >
          ...
          {error && (
            <div className="mt-6 p-4 bg-red-50 border border-red-100 rounded-lg text-red-700 text-sm flex items-center gap-2">
        ...
        {result && (
          <section id="results" className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-500">
            {/* Tabs Navigation */}
            <div className="flex flex-wrap justify-center gap-2 p-1 bg-white border border-slate-200 rounded-2xl w-fit mx-auto shadow-sm sticky top-4 z-30 mb-12">
              {[
                { id: 'core', label: 'Identity Core', icon: '💎' },
                { id: 'strategy', label: 'Strategy & Persona', icon: '🧠' },
                { id: 'visual', label: 'Visual Studio', icon: '🎨' },
                { id: 'marketing', label: 'Marketing Kit', icon: '🚀' }
              ].map(tab => (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id as any)}
                  className={`px-6 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === tab.id ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}
                >
                  <span>{tab.icon}</span>
                  {tab.label}
                </button>
            ...
            {activeTab === 'core' && (
              <div className="space-y-8">
                <div className="bg-indigo-900 rounded-3xl p-8 md:p-12 text-white shadow-2xl relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <span className="text-xs font-bold uppercase tracking-widest text-indigo-400 mb-2 block">Brand Positioning</span>
                    <h2 className="text-3xl md:text-5xl font-bold mb-6 leading-tight max-w-4xl tracking-tight italic">
                  ...
                  <Card title="Brand Names & Logic" icon={<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/></svg>}>
                    <div className="space-y-4">
                      {result.brandNames.map((bn, i) => (
                        <div key={i} className="p-4 bg-slate-50 rounded-2xl border border-slate-200 group hover:border-indigo-400 transition-all cursor-pointer">
                  ...
                  <Card title="Strategic Taglines" icon={<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>}>
                    <div className="space-y-3">
                      {result.taglines.map((tag, i) => (
                        <div key={i} className="p-4 border border-slate-100 rounded-2xl text-slate-800 font-medium hover:bg-indigo-50 hover:border-indigo-200 transition-all cursor-pointer flex items-center gap-3">
                          <span className="w-6 h-6 bg-indigo-100 text-indigo-600 rounded-full flex items-center justify-center text-[10px] font-bold">{i+1}</span>
                          "{tag}"
                        </div>
                      ))}
                    </div>
            ...
            {activeTab === 'strategy' && (
              <div className="space-y-12">
                ...
                <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl overflow-hidden">
                  <div className="bg-slate-900 p-8 text-white flex flex-col md:flex-row justify-between items-start md:items-center">
                    <div>
                      <span className="text-xs font-bold uppercase tracking-widest text-indigo-400 mb-1 block">Ideal Customer Profile</span>
                      <h3 className="text-3xl font-extrabold">{result.customerPersona.name}</h3>
                      <p className="text-slate-400 font-medium">{result.customerPersona.ageRange} • {result.customerPersona.lifestyle}</p>
                    </div>
                    <div className="mt-4 md:mt-0 px-5 py-2 bg-indigo-600 rounded-full text-xs font-bold tracking-widest uppercase">Target Persona</div>
                  </div>
                  <div className="p-8 grid grid-cols-1 md:grid-cols-3 gap-12">
                    <div className="space-y-4">
                      <h4 className="font-bold text-slate-900 flex items-center gap-2 text-sm uppercase tracking-wider">
                        <span className="w-8 h-8 bg-emerald-100 text-emerald-600 rounded-xl flex items-center justify-center">🎯</span> Core Goals
                      </h4>
                    ...
                    <div className="space-y-4">
                      <h4 className="font-bold text-slate-900 flex items-center gap-2 text-sm uppercase tracking-wider">
                        <span className="w-8 h-8 bg-red-100 text-red-600 rounded-xl flex items-center justify-center">⚠️</span> Pain Points
                      </h4>
                      <ul className="text-sm text-slate-600 space-y-3">
                    ...
                    <div className="space-y-4">
                      <h4 className="font-bold text-slate-900 flex items-center gap-2 text-sm uppercase tracking-wider">
                        <span className="w-8 h-8 bg-indigo-100 text-indigo-600 rounded-xl flex items-center justify-center">📱</span> Discovery
                      </h4>
                      <ul className="text-sm text-slate-600 space-y-3">
                  ...
                  <Card title="Brand Archetype" icon="🏛️">
                    <div className="space-y-4">
                      <div className="p-6 bg-indigo-50 rounded-2xl border border-indigo-100 text-center">
                        ...
                        <div className="text-2xl font-black text-indigo-700 uppercase">{result.personalityProfile.archetype}</div>
                      </div>
                      <p className="text-sm text-slate-600 leading-relaxed italic text-center px-4">
                        "Your customers will experience an emotional response of <strong>{result.personalityProfile.emotionalResponse}</strong> when interacting with this brand."
                      </p>
                    </div>
                  </Card>
                  <Card title="Voice & Tone Guide" icon="🎙️">
                    <div className="space-y-4 text-sm">
                      <div className="mb-4">
                        ...
                        <p className="text-slate-600 bg-slate-50 p-3 rounded-xl border border-slate-100">{result.brandVoice.style}</p>
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div className="p-3 bg-emerald-50 rounded-2xl border border-emerald-100">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {result.brandVoice.wordsToUse.map((w, i) => <span key={i} className="bg-white px-2 py-0.5 rounded-md text-[11px] font-bold text-emerald-700">{w}</span>)}
                          </div>
                        </div>
                        <div className="p-3 bg-red-50 rounded-2xl border border-red-100">
                          <span className="text-[10px] font-bold text-red-600 block mb-2 uppercase tracking-widest">Forbidden</span>
                          <div className="flex flex-wrap gap-1">
                            {result.brandVoice.wordsToAvoid.map((w, i) => <span key={i} className="bg-white px-2 py-0.5 rounded-md text-[11px] font-bold text-red-700">{w}</span>)}
                          </div>
                        </div>
                      </div>
                      <div className="pt-2">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-2">Writing Rules</span>
                        <ul className="space-y-1 text-xs text-slate-500">
                          {result.brandVoice.rules.map((r, i) => <li key={i}>• {r}</li>)}
                        </ul>
                      </div>
                    </div>
                  </Card>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {result.colorPalette.map((color, i) => (
                      <div key={i} className="flex flex-col items-center text-center gap-4 group">
                        <div 
                          className="w-full h-32 rounded-3xl shadow-xl border border-white flex items-center justify-center text-[10px] font-bold text-white group-hover:scale-[1.02] transition-transform" 
                          style={{ backgroundColor: color.hex }}
                        >
                          <span className="bg-black/20 px-3 py-1.5 rounded-lg backdrop-blur-sm text-sm tracking-wider">{color.hex}</span>
                        </div>
                        <div>
              ...
              <div className="space-y-12">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  {Object.entries(result.socialBio).map(([platform, bio]) => (
                    <div key={platform} className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex items-center gap-2 mb-4">
                        <span className="p-2 bg-slate-100 rounded-xl text-xs font-bold uppercase tracking-widest text-slate-400">{platform}</span>
                      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="bg-white rounded-[2rem] border border-slate-200 shadow-xl overflow-hidden group">
                    ...
                    <div className="p-8 space-y-6">
                      <div className="p-5 bg-indigo-50/50 rounded-2xl text-indigo-800 text-sm font-semibold border border-indigo-100 italic">
                      ...
                      <div className="text-sm text-slate-600 leading-relaxed whitespace-pre-wrap relative">
                        ...
                        <button onClick={() => navigator.clipboard.writeText(result.socialStarterKit.launchPost.caption)} className="absolute top-0 right-0 bg-slate-100 p-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity">📋</button>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-[2rem] border border-slate-200 shadow-xl overflow-hidden group">
                    ...
                    <div className="p-8 space-y-6">
                      <div className="p-5 bg-emerald-50/50 rounded-2xl text-emerald-800 text-sm font-semibold border border-emerald-100 italic">
                      ...
                      <div className="text-sm text-slate-600 leading-relaxed whitespace-pre-wrap relative">
                        ...
                        <button onClick={() => navigator.clipboard.writeText(result.socialStarterKit.engagementPost.caption)} className="absolute top-0 right-0 bg-slate-100 p-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity">📋</button>
                      </div>
                    </div>
                  </div>
                </div>

                <div className="p-10 bg-white border border-slate-200 rounded-[2rem] shadow-sm">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {result.socialStarterKit.hashtags.map((h, i) => (
                      <span key={i} className="px-4 py-2 bg-slate-50 text-slate-500 rounded-xl text-sm font-bold border border-slate-200 hover:border-indigo-300 hover:text-indigo-600 transition-all cursor-pointer">
                        {h.startsWith('#') ? h : `#${h}`}
                      </span>
                    ))}
                  </div>
      ...
      <footer className="mt-20 border-t border-slate-200 py-16 text-center text-slate-400 bg-white">
